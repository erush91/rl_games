...
class BasePlayer(object):
    ...
    def __init__(self, params):
        ...
        self.import_data_path = self.env.cfg['env'].get('import_data_path', '')
        self.export_data = self.env.cfg['env'].get('export_data', False)
        self.export_data_path = self.env.cfg['env'].get('export_data_path', '')
        self.export_data = self.env.cfg['env'].get('export_data', False) 
        ...
        self.ablation_scl_pca_path = self.env.cfg['env']['ablate'].get('scl_pca_path', '')
        self.random_ablations_obs_in = self.env.cfg['env']['ablate']['random'].get('obs_in', 0)
        self.random_ablations_hn_out = self.env.cfg['env']['ablate']['random'].get('hn_out', 0)
        self.random_ablations_hn_in = self.env.cfg['env']['ablate']['random'].get('hn_in', 0)
        self.random_ablations_cn_in = self.env.cfg['env']['ablate']['random'].get('cn_in', 0)
        self.targeted_ablations_obs_in = self.env.cfg['env']['ablate']['targeted'].get('obs_in', 0)
        self.targeted_ablations_hn_out = self.env.cfg['env']['ablate']['targeted'].get('hn_out', 0)
        self.targeted_ablations_hn_in = self.env.cfg['env']['ablate']['targeted'].get('hn_in', 0)
        self.targeted_ablations_cn_in = self.env.cfg['env']['ablate']['targeted'].get('cn_in', 0)




    #     # Example usage
    #     zero_indices_str = sys.argv[1]  # Assuming the indices are passed as the first argument
    #     zero_indices = parse_indices(zero_indices_str)

    #     # Assuming 'neural_obs_override' and 'ROBOT_ABLATION_IDX_FOR_MASK2' are defined
    #     # Here, you'd set the specified indices to zero
    #     for index in zero_indices:
    #         neural_obs_override[ROBOT_ABLATION_IDX_FOR_MASK2, index] = 0
    ...
    #     else:
    #         # Single index or multiple indices separated by commas
    #         return [int(idx) for idx in indices_str.split(',') if idx.strip().isdigit()]
    ...
    def run(self):
        ...
        for _ in range(n_games):
            ...
            if self.random_ablation_trial or self.targeted_ablation_trial:

                pp =  self.ablation_scl_pca_path
                pp = Path().resolve() / pp
                
                # load scaler and pca transforms
                scl_hc = pk.load(open(pp / 'A_LSTM_HC_SCL.pkl','rb'))
                pca_hc = pk.load(open(pp / 'A_LSTM_HC_PCA.pkl','rb'))

                scl_obs = pk.load(open(pp / 'OBS_SCL.pkl','rb'))
                pca_obs = pk.load(open(pp / 'OBS_PCA.pkl','rb'))
                ...
                if self.random_ablation_trial:

                    ablate_obs_in_np[np.arange(N_ROBOTS).reshape(-1, 1), row_indices2[:, :DIM_OBS+DIM_ACT-self.random_ablations_obs_in]] = torch.nan
                    ablate_obs_in = torch.tensor(ablate_obs_in_np, dtype=torch.float, device='cuda')

                    ablate_hn_out_np[np.arange(N_ROBOTS).reshape(-1, 1), row_indices[:, :DIM_A_LSTM_HX-self.random_ablations_hn_out]] = torch.nan
                    ablate_hn_out = torch.tensor(ablate_hn_out_np, dtype=torch.float, device='cuda').unsqueeze(0)

                    ablate_hn_in_np[np.arange(N_ROBOTS).reshape(-1, 1), row_indices[:, :DIM_A_LSTM_HX-self.random_ablations_hn_in]] = torch.nan
                    ablate_hn_in = torch.tensor(ablate_hn_in_np, dtype=torch.float, device='cuda').unsqueeze(0)

                    ablate_cn_in_np[np.arange(N_ROBOTS).reshape(-1, 1), row_indices[:, :DIM_A_LSTM_CX-self.random_ablations_cn_in]] = torch.nan
                ...
                if self.targeted_ablation_trial:
                    ...
                    hn_out_idx_ablated = hn_out_idx_by_ascending_gradient[:self.targeted_ablations_hn_out]
                    hn_out_idx_not_ablated = hn_out_idx_by_ascending_gradient[self.targeted_ablations_hn_out:]
                    ...
                    hn_in_idx_ablated = hn_in_idx_by_ascending_gradient[:self.targeted_ablations_hn_in]
                    hn_in_idx_not_ablated = hn_in_idx_by_ascending_gradient[self.targeted_ablations_hn_in:]
                    ...
                    cn_in_idx_ablated = cn_in_idx_by_ascending_gradient[:self.targeted_ablations_cn_in]
                    cn_in_idx_not_ablated = cn_in_idx_by_ascending_gradient[self.targeted_ablations_cn_in:]
            ...
            for t in range(self.max_steps - 1):
                ...
                if done_count > 0:
                    ...
                    if batch_size//self.num_agents == 1 or games_won >= n_games:
                        break
        ...
        print(sum_rewards)
        # if print_game_res:
        #     print('av reward:', sum_rewards / games_won * n_game_life, 'av steps:', sum_steps /
        #           games_won * n_game_life, 'winrate:', sum_game_res / games_won * n_game_life)
        # else:
        #     print('av reward:', sum_rewards / games_won * n_game_life,
                #   'av steps:', sum_steps / games_won * n_game_life)
