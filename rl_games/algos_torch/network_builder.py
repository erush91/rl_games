            ...
            with torch.enable_grad():
                ...
                if self.separate:
                    ...
                    if self.has_rnn:
                        ...
                        a_out_temp6 = a_out_temp5.contiguous().reshape(a_out_temp5.size()[0] * a_out_temp5.size()[1], -1)
                        c_out_temp6 = c_out_temp5.contiguous().reshape(c_out_temp5.size()[0] * c_out_temp5.size()[1], -1)
                        if self.rnn_ln:
                            a_out_temp = self.a_layer_norm(a_out_temp)
                            c_out_temp = self.c_layer_norm(c_out_temp)
                        ...
                        if self.is_rnn_before_mlp:
                            a_out_temp = self.actor_mlp(a_out_temp)
                            a_out_temp = self.critic_mlp(c_out_temp)
                    else:
                        a_out_temp = self.actor_mlp(a_out_temp)
                        c_out_temp = self.critic_mlp(c_out_temp)
                                
                    value = self.value_act(self.value(c_out_temp6))

                    if self.is_discrete:
                        logits = self.logits(a_out_temp6)
                        return logits, value, states_out, self.selected_out

                    if self.is_multi_discrete:
                        logits = [logit(a_out_temp6) for logit in self.logits]
                        return logits, value, states_out, self.selected_out

                    if self.is_continuous:
                        mu = self.mu_act(self.mu(a_out_temp6))
                        if self.fixed_sigma:
